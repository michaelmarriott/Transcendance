...
void Ripple(int speed) {
  ...
  for (int x = 0; x < NUM_STRIPS; x++) {
    for (uint16_t i = 0; i < NUM_LEDS_PER_STRIP; i++) {
      leds[(x * NUM_LEDS_PER_STRIP) + i] = CHSV(90, ripple_currentBg, 15);         // strip.setPixelColor(l, Wheel(currentBg, 0.1));
    }
  }
...
CRGB ChangeColor() {
  random16_add_entropy(random());
  return CRGB(random8(0,200), random8(0,200), random8(0,200));
}
